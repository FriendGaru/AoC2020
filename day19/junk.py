...
def match_rules(rule_id_list, remaining_message):
    ...
    if isinstance(next_rule_content, str):
        ...
        assert(isinstance(next_rule_content, list))
        permutations = positional_permutatons(next_rule_content)
        remaining_rule_ids = list(rule_id_list[1:])
        for permutation in permutations:
            permutation.extend(remaining_rule_ids)
